...
apt-get -q update \
 && apt-get -q -y install \
        ...
        mercurial git \
        llvm-3.6 libclang-3.6-dev clang-3.6 \
...
update-alternatives --force --install /usr/local/bin/llvm-config llvm-config /usr/bin/llvm-config-3.6 0
# There is no clang++ until we do this:
update-alternatives --force --install /usr/local/bin/clang++ clang++ /usr/bin/clang++-3.6 0
