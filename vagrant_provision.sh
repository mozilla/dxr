...
# down.
# Alternately, per https://github.com/TryGhost/Ghost-Vagrant#updating-virtual-box-guest-additions (from outside) the guest run:
# 1) vagrant plugin install vagrant-vbguest
# 2) vagrant up --no-provision
# 3) vagrant ssh -c 'sudo apt-get -y -q purge virtualbox-guest-dkms virtualbox-guest-utils virtualbox-guest-x11'
# 4) vagrant halt
