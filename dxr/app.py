...
from flask import (Blueprint, Flask, send_from_directory, current_app,
                   send_file, request, redirect, jsonify, render_template, Markup)
...
from dxr.repos import *
from pygments import highlight
from pygments.lexers import get_lexer_by_name, guess_lexer, guess_lexer_for_filename, TextLexer
...
def _vcs_permalink_file(vcs_folder, vcs_type, revision, file):
    try:
        # create the VCS object, which scans for underlying .hg/.git repos
        repo = load(VCS(vcs_folder))
        # now get the file from Git or Mercurial
        code = repo[vcs_folder].get_file_revision((revision, file))
    except:
        code = None
...
def permalink(repository, revision, file):
    config = current_app.config
    wwwroot = config['WWW_ROOT']
    vcs_folder = config['TREES'][repository]['vcs_folder']
    vcs_type = config['TREES'][repository]['vcs_type']
    error = ''
    code = ''
    status_code = ''

    if vcs_folder is not None:
        try:
            code = _vcs_permalink_file(vcs_folder, vcs_type, revision, file)
            if code == None:
                status_code = 404
            else:
                status_code = 200
        ...
        if status_code == 200:
            try:
                try:
                    # Guess for filename is a bit faster than guess_lexer, and it doesn't OOM!
                    lexer = guess_lexer_for_filename(file, code)
                except:
                    # If no lexer is detected, return plaintext result. Better than nothing!
                    lexer = TextLexer()
                # Format lines with <br> instead of \n or blank spaces. Need this for line splitting later
                formatter = HtmlFormatter(lineseparator='<br>')
                code = highlight(code, lexer, formatter)
                # Strip out the div/pre markup and add it via the template instead
                # Otherwise the formatter markup goes wonky in the template's for loop
                # This is easier than subclassing the HtmlFormatter to not return the div/pre tags
                code = code.replace('<div class="highlight"><pre>', '')
                code = code.replace('</pre></div>', '')
                # Split the rendered html into lines on <br> elements for calculating line numbers
                code = code.split('<br>')
            except: #Any old except, some parsing broke somewhere. Let's not be picky in how it's handled
            ...
            if status_code is None:
                status_code = 500
            if error is None:
                error = "Something in the lexer for %s broke. " % file
    ...
    arguments = {
        ...
        'wwwroot': wwwroot,
        'name': '%s@%s permalink' % (file, revision),
        'lines': code,
        'filename': file,
        'error': error,
    ...
    if error or status_code != 200:
        ...
        return render_template('permalink.html', **arguments)
